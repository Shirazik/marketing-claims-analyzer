 ...
 * API keys are encrypted at rest via AES-GCM with a PBKDF2-derived key
 * (see lib/crypto.js). Existing plaintext keys are migrated transparently
 * on first read.
 */
...
async function getApiKey(provider = null) {
  ...
  const stored = await get(getProviderConfig(activeProvider).key);
  if (!stored) return null;
  if (typeof stored === 'string') {
    // Legacy plaintext key — migrate to encrypted format transparently
    await setApiKey(stored, activeProvider);
    return stored;
  }
  try {
    return await decryptValue(stored);
  } catch {
    // Decryption failed (e.g. different Google account) — signal missing key
    return null;
  }
}
...
async function setApiKey(key, provider = null) {
  ...
  return set(getProviderConfig(activeProvider).key, encrypted);
}
